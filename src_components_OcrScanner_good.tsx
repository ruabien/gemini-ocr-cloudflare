...
const handleSelectedFiles = async (files: File[]) => {
  ...
  if (pdfFilesInSelection.length > 0 && totalPdfCount > 1) {
    alert("Hß╗ç thß╗æng chß╗ë hß╗ù trß╗ú xß╗¡ l├╜ mß╗ùi lß║ºn 1 tß╗çp PDF. Vui l├▓ng tß║úi l├¬n tß╗½ng tß╗çp mß╗Öt ─æß╗â ─æß║úm bß║úo hiß╗çu n─âng.");
    return;
  }
  ...
  for (const qFile of newQueued) {
    if (qFile.file.name.toLowerCase().endsWith('.pdf')) {
      try {
        ...
        console.error("Lß╗ùi ph├ón t├ích PDF khi tß║úi tß╗çp:", err);
      }
    }
  }
};
  ...
  const deleteQueuedFile = (id: string, fileName: string) => {
    if (isBatchProcessing) {
      alert("Kh├┤ng thß╗â x├│a khi ─æang OCR");
      return;
    }
  ...
  const resetAll = () => {
    ...
    const confirmReset = window.confirm("Bß║ín c├│ chß║»c chß║»n muß╗æn x├│a tß║Ñt cß║ú danh s├ích v├á l├ám lß║íi tß╗½ ─æß║ºu?");
...
const startOcrProcess = async () => {
  ...
  // ---- PreΓÇæflight usage checks for FREE users ----
  let isPro = false;
  let currentUsage = 0;
  if (user?.uid) {
    try {
      ...
      if (checkRes.ok) {
        ...
        if (!isPro && remaining <= 10) {
          setSoftBanner(
            `H├┤m nay bß║ín ─æ├ú sß╗¡ dß╗Ñng ${currentUsage} / 50 trang miß╗àn ph├¡. Bß║ín c├▓n ${remaining} trang. N├óng cß║Ñp LexOCR PRO ─æß╗â xß╗¡ l├╜ kh├┤ng giß╗¢i hß║ín.`
          );
        } else {
          setSoftBanner(null);
        }
  ...
  if (!isPro) {
    ...
    if (totalRequestedPages > 20) {
      setLimitModal({
        ...
        requested: totalRequestedPages,
        onAccept: async () => {
          // "Tiß║┐p tß╗Ñc vß╗¢i 20 trang ─æß║ºu": set range 1-20
          setLimitModal(null);
          setFromPage("1");
          setToPage("20");
...
/* Final pool ΓÇô remove empty, null, undefined and duplicates */
...
if (geminiKeyPool.length === 0) {
  setErrorModalMsg(
    <span>
      ...
      >
        Xem h╞░ß╗¢ng dß║½n tß║ío trong 3 ph├║t.
      </a>
    </span>
  );
...
if (import.meta.env.DEV) console.info(`--- EXECUTING SINGLE OCR RUN FOR ${file.type?.startsWith("image/") ? `Image_1_${file.name || "unknown"}` : `Page_${pageNum}` } ---`);
      ...
      const runOcrSpaceFallback = (): Promise<string> => {
        return new Promise<string>((resolveFallback, rejectFallback) => {
          ...
          setBatchProgressText(`Gemini qu├í tß║úi, chuyß╗ân sang OCR dß╗▒ ph├▓ng - Trang ${pageNum}...`);
...
// Log request params before OCR.space fallback
                  ...
                  .then(async res => {
                    ...
                    if (data.ParsedResults?.[0]?.ParsedText) {
                      ...
                      const msg = "Kh├┤ng nhß║¡n dß║íng ─æ╞░ß╗úc v─ân bß║ún n├áo tß╗½ phß║ún hß╗ôi cß╗ºa OCR.space.";
                      setEditorContent((prev) => prev + (prev ? "\n\n" : "") + msg);
                      editorContentRef.current += (editorContentRef.current ? "\n\n" : "") + msg;
              ...
              img.onerror = () => {
                rejectFallback(new Error("Kh├┤ng thß╗â tß║úi ß║únh cß║Ñu h├¼nh canvas ─æß╗â n├⌐n cho OCR.space."));
              };
...
if (import.meta.env.DEV) console.info(`[OCR ${file.type?.startsWith("image/") ? `Image_1_${file.name || "unknown"}` : `Page_${pageNum}`} START] ${Date.now()}`);
          setBatchProgressText(`─Éang thß╗¡ Gemini key ${activeKeyIndex + 1}/${geminiKeyPool.length} - Trang ${pageNum}...`);
...
if (xhr.status === 429) {
  // Gemini quota exceeded ΓÇô move to next key
  // @ts-ignore
  if (import.meta.env.DEV) console.info(`Gemini key ${activeKeyIndex + 1}/${geminiKeyPool.length} received 429, switching key`);
  setBatchProgressText(`Gemini key ${activeKeyIndex + 1} qu├í tß║úi, thß╗¡ key ${activeKeyIndex + 2}/${geminiKeyPool.length} - Trang ${pageNum}...`);
  reject({ status: 429 });
  return;
} else if (xhr.status === 401 || xhr.status === 403) {
  // Auth error ΓÇô skip this key
  // @ts-ignore
  if (import.meta.env.DEV) console.info(`Gemini key ${activeKeyIndex + 1}/${geminiKeyPool.length} received ${xhr.status}, skipping`);
  setBatchProgressText(`Gemini key ${activeKeyIndex + 1} kh├┤ng hß╗úp lß╗ç (${xhr.status}), thß╗¡ key ${activeKeyIndex + 2}/${geminiKeyPool.length} - Trang ${pageNum}...`);
  reject({ status: xhr.status });
  return;
} else if (isTransientError) {
              if (!isRetry) {
                console.warn(`[OCR WARN] Transient error HTTP ${xhr.status} at page ${pageNum}. Retrying once in 2.5s...`);
                setBatchProgressText(`Gemini gß║╖p lß╗ùi tß║ím thß╗¥i (${xhr.status}), ─æang thß╗¡ lß║íi - Trang ${pageNum}...`);
                setTimeout(() => {
              ...
              try {
                ...
                if (lines.length > 0) {
                  ...
                  const isAiIntro = /^(D╞░ß╗¢i ─æ├óy l├á|V─ân bß║ún ─æ├ú ─æ╞░ß╗úc|Kß║┐t quß║ú|─É├óy l├á v─ân bß║ún)/i.test(firstLine) && firstLine.endsWith(':');
                  if (isAiIntro) {
                    lines.shift();
                  }
                }
...
if (import.meta.env.DEV) console.info(`[OCR ${file.type?.startsWith("image/") ? `Image_1_${file.name || "unknown"}` : `Page_${pageNum}`} END] ${Date.now()}`);
          ...
          xhr.onerror = () => {
            if (!isRetry) {
              ...
              setBatchProgressText(`Lß╗ùi kß║┐t nß╗æi mß║íng, ─æang thß╗¡ lß║íi - Trang ${pageNum}...`);
              setTimeout(() => {
                makeRequest(activeKey, true).then(resolve).catch(reject);
              }, 2500);
            } else {
          ...
          const payload = {
            "contents": [{
              "parts": [
                {"text": "Tr├¡ch xuß║Ñt ch├¡nh x├íc 100% to├án bß╗Ö nß╗Öi dung v─ân bß║ún c├│ trong h├¼nh ß║únh n├áy sang tiß║┐ng Viß╗çt. Giß╗» nguy├¬n ─æß╗ïnh dß║íng, kh├┤ng th├¬m bß╗¢t bß║Ñt kß╗│ tß╗½ ngß╗» hay lß╗¥i giß║úi th├¡ch n├áo."},
                {"inlineData": {
...
/* Try each Gemini key in order ΓÇô only fall back to OCR.space when all keys are exhausted */
while (true) {
  const currentKey = geminiKeyPool[activeKeyIndex];
  if (!currentKey) {
    // No more keys ΓÇô use OCR.space fallback
    // @ts-ignore
    if (import.meta.env.DEV) console.info('All Gemini keys exhausted, falling back to OCR.space');
    setBatchProgressText(`Tß║Ñt cß║ú Gemini key qu├í tß║úi, chuyß╗ân sang OCR dß╗▒ ph├▓ng - Trang ${pageNum}...`);
  ...
  try {
    ...
    for (let i = 0; i < filesToProcess.length; i++) {
      ...
      // Update UI status for this specific index to "─Éang b├│c t├ích"
      ...
      const processSinglePage = async (pageFile: File, pageIdx: number, totalPages: number) => {
        ...
        try {
          ...
          const errorMsg = `[Lß╗ùi b├│c t├ích Trang ${pageIdx}]: ${errorMsgObj}`;
          setEditorContent((prev) => prev + (prev ? "\n\n" + errorMsg : errorMsg));
          editorContentRef.current += (editorContentRef.current ? "\n\n" + errorMsg : errorMsg);
        } finally {
      ...
      if (isPdf) {
        ...
        if (pageFiles.length === 0) {
          try {
            ...
            setFileErrors(prev => ({ ...prev, [i]: "[Lß╗ùi b├│c t├ích]: Vui l├▓ng kiß╗âm tra lß║íi chß║Ñt l╞░ß╗úng tß╗çp tin hoß║╖c cß║Ñu h├¼nh Key cß╗ºa trang n├áy." }));
        ...
        for (let p = startPage; p <= endPage; p++) {
          const pageFile = pageFiles[p - 1]; // zeroΓÇæbased index
          setBatchProgressText(`─Éang b├│c t├ích ${file.name} - Trang ${p}/${endPage}...`);
        ...
        // NonΓÇæPDF file: process normally with a single request
        try {
          ...
          setQueuedFiles(prev => {
            ...
            if (currentFile?.pageStates?.[1]?.status === 'error') {
              ...
              setFileErrors(errs => ({
                ...errs,
                [i]: "[Lß╗ùi b├│c t├ích]: Vui l├▓ng kiß╗âm tra lß║íi chß║Ñt l╞░ß╗úng tß╗çp tin hoß║╖c cß║Ñu h├¼nh Key cß╗ºa trang n├áy."
              }));
            } else {
              updateFileStatus(i, "completed");
            }
    ...
    setTimeout(async () => {
        ...
        try {
          ...
          queuedFiles.forEach(q => {
            if (q.pageStates) {
              ...
              // SingleΓÇæimage file counted as one successful page
              successfulPages += 1;
            }
          });
          ...
          if (successfulPages > 0) {
            ...
            if (!response.ok || !data || !data.success) {
              ...
              if (code === "quota_exceeded") {
                 setErrorModalMsg("Kß║┐t quß║ú OCR ─æ├ú ho├án tß║Ñt, nh╞░ng hß║ín mß╗⌐c miß╗àn ph├¡ h├┤m nay ─æ├ú ─æ╞░ß╗úc sß╗¡ dß╗Ñng ─æß╗ông thß╗¥i ß╗ƒ phi├¬n kh├íc.");
                 setPagesUsedToday(data?.pagesUsed || 50);
                 const remaining = data?.remainingPages || 0;
                 if (remaining <= 0) {
                   setSoftBanner("Bß║ín ─æ├ú sß╗¡ dß╗Ñng hß║┐t hß║ín mß╗⌐c miß╗àn ph├¡ h├┤m nay. N├óng cß║Ñp LexOCR PRO ─æß╗â xß╗¡ l├╜ kh├┤ng giß╗¢i hß║ín.");
                 } else {
                   setSoftBanner(`H├┤m nay bß║ín ─æ├ú sß╗¡ dß╗Ñng ${data?.pagesUsed} / 50 trang miß╗àn ph├¡. Bß║ín c├▓n ${remaining} trang. N├óng cß║Ñp LexOCR PRO ─æß╗â xß╗¡ l├╜ kh├┤ng giß╗¢i hß║ín.`);
                 }
              } else if (code === "transaction_failed" || code === "server_error") {
                 console.error("Usage commit failed safely.", data?.message || "server error");
                 setSoftBanner("Kß║┐t quß║ú OCR ─æ├ú ho├án tß║Ñt nh╞░ng hß╗ç thß╗æng ch╞░a thß╗â cß║¡p nhß║¡t hß║ín mß╗⌐c sß╗¡ dß╗Ñng. Vui l├▓ng thß╗¡ lß║íi sau.");
              } else if (response.status === 400) {
                 setErrorModalMsg(data?.message || "Dß╗» liß╗çu kh├┤ng hß╗úp lß╗ç. Kß║┐t quß║ú OCR vß║½n ─æ╞░ß╗úc giß╗» nguy├¬n.");
              } else {
                 setSoftBanner("Kß║┐t quß║ú OCR ─æ├ú ho├án tß║Ñt nh╞░ng hß╗ç thß╗æng ch╞░a thß╗â cß║¡p nhß║¡t hß║ín mß╗⌐c sß╗¡ dß╗Ñng. Vui l├▓ng thß╗¡ lß║íi sau.");
              }
            } else {
              ...
              if (!data.isPro && remaining <= 10) {
                 setSoftBanner(`H├┤m nay bß║ín ─æ├ú sß╗¡ dß╗Ñng ${pagesUsed} / 50 trang miß╗àn ph├¡. Bß║ín c├▓n ${remaining} trang. N├óng cß║Ñp LexOCR PRO ─æß╗â xß╗¡ l├╜ kh├┤ng giß╗¢i hß║ín.`);
              } else {
                 setSoftBanner(null);
              }
            }
      ...
      if (finalContent && finalContent.trim() !== "") {
        ...
        onFileLoaded({ 
          name: filesToProcess.length > 1 && autoMerge ? "Hß╗ô S╞í Gß╗Öp Nhiß╗üu T├ái Liß╗çu" : (filesToProcess[0]?.file?.name || "T├ái Liß╗çu OCR"), 
    ...
    <div id="ocr-scanner-tab" className="flex flex-col space-y-4 pb-12 bg-slate-50">
        ...
        <div className="border-b border-slate-200 pb-3">
          <h2 className="text-xl sm:text-2xl font-bold tracking-tight text-slate-900 flex items-center space-x-2">
            <span>Sß╗æ h├│a & Tr├¡ch xuß║Ñt hß╗ô s╞í vß╗Ñ ├ín chuy├¬n s├óu</span>
          </h2>
          <p className="text-slate-500 text-xs sm:text-sm mt-0.5">
            Tß║úi t├ái liß╗çu tß╗æ tß╗Ñng ─æß╗ïnh dß║íng PDF, JPEG, PNG. Hß╗ù trß╗ú <strong>tß║úi l├¬n h├áng loß║ít nhiß╗üu tß╗çp</strong> (Bulk Image OCR) v├á tß╗▒ ─æß╗Öng gß╗Öp v─ân bß║ún xuß║Ñt chuß║⌐n.
          </p>
        </div>

        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
          
          {/* Cß║ñU H├îNH OCR V├Ç DROPZONE CH├ìNH (Chiß║┐m 2 cß╗Öt tr├¬n m├án h├¼nh Desktop) */}
          <div className="lg:col-span-2 space-y-6">
            ...
            >
              ...
              <h3 className="font-bold text-slate-800 text-sm">
                K├⌐o thß║ú nhiß╗üu t├ái liß╗çu v├áo ─æ├óy hoß║╖c click ─æß╗â duyß╗çt tß╗½ m├íy t├¡nh
              </h3>
              <p className="text-[11px] text-slate-500 mt-1 max-w-xl">
                Hß╗ù trß╗ú chß╗ìn nhiß╗üu tß╗çp tin c├╣ng l├║c. Bß║ún qu├⌐t sß║╜ ─æ╞░ß╗úc tß╗▒ ─æß╗Öng ph├ón lß╗¢p, ─æ╞░a v├áo h├áng ─æß╗úi v├á b├│c t├ích nß╗Öi dung tuß║ºn tß╗▒.
              </p>
              ...
              <div className="mt-2.5 flex items-center justify-center space-x-2 text-[10px] text-slate-400 font-semibold font-mono bg-slate-50 border border-slate-150 rounded px-2.5 py-0.5">
                ...
                <span>Mß║¼T M├â HO├ü TR├èN THIß║╛T Bß╗è ─Éß║ªU CUß╗ÉI</span>
              </div>
            </div>

            {/* N├ÜT Bß║«T ─Éß║ªU TR├ìCH XUß║ñT OCR */}
            <div className="flex justify-center w-full">
              ...
              >
                ...
                <span>
                  {isBatchProcessing 
                    ? "─Éang b├│c t├ích hß╗ô s╞í..." 
                    : isSlicing 
                      ? "─Éang ph├ón t├ích PDF..." 
                      : "Bß║»t ─æß║ºu b├│c t├ích hß╗ô s╞í"}
                </span>
              </button>
            </div>
          ...
          {/* Cß║ñU H├îNH Hß╗å THß╗ÉNG (Cß╗Öt b├¬n phß║úi Desktop) */}
          <div className="space-y-6">
            <div className="bg-slate-900 border border-slate-800 rounded-xl p-5 shadow-xl text-slate-100 relative overflow-hidden">
              ...
              <h4 className="font-bold text-xs uppercase tracking-widest text-slate-100 flex items-center mb-4 relative z-10">
                ...
                <span>ΓÜÖ∩╕Å Cß║ñU H├îNH Hß╗å THß╗ÉNG</span>
              </h4>

              <div className="relative z-10 space-y-4">
                {/* CHß╗îN Mß╗ñC ─É├ìCH Xß╗¼ L├¥ */}
                <div className="space-y-2 border-b border-slate-800 pb-4">
                  <span className="block text-[11px] font-bold text-slate-300 uppercase tracking-wider">≡ƒÄ» Chß╗ìn mß╗Ñc ─æ├¡ch xß╗¡ l├╜</span>
                  <div className="space-y-2">
                    <label className={`block p-2.5 rounded-lg border cursor-pointer transition-all ${
                      ...
                      <div className="flex items-center space-x-2">
                        ...
                        />
                        <span className="text-xs font-bold">Chuyß╗ân File ΓåÆ Text/Word</span>
                      </div>
                      <p className="text-[10px] text-slate-400 mt-1 pl-6 leading-relaxed">
                        D├ánh cho Bß║ún ├ín, C├ío trß║íng, Kß║┐t luß║¡n ─æiß╗üu tra, Bi├¬n bß║ún ghi lß╗¥i khai.
                      </p>
                    </label>
                    ...
                    <label className={`block p-2.5 rounded-lg border cursor-pointer transition-all ${
                      ...
                      <div className="flex items-center space-x-2">
                        <input
                          ...
                          className="text-red-600 focus:ring-red-500 h-4 w-4 bg-slate-800 border-slate-600"
                        />
                        <span className="text-xs font-bold">Tr├¡ch xuß║Ñt dß╗» liß╗çu ΓåÆ Excel</span>
                      </div>
                      <p className="text-[10px] text-slate-400 mt-1 pl-6 leading-relaxed">
                        D├ánh cho Th├┤ng b├ío thß╗Ñ l├╜, Quyß║┐t ─æß╗ïnh khß╗ƒi tß╗æ bß╗ï can v├á c├íc biß╗âu mß║½u cß║ºn xuß║Ñt bß║úng.
                      </p>
                    </label>
                  </div>
                </div>

                {/* PHß║áM VI TR├ìCH XUß║ñT (PAGE RANGE) */}
                <div>
                  <label className="block text-[10px] font-bold text-slate-300 mb-1.5 uppercase tracking-wide">
                    Phß║ím vi tr├¡ch xuß║Ñt (Page Range)
                  </label>
                  ...
                  <p className="text-[9px] text-slate-400 mt-1.5">*Chß╗ë khß║ú dß╗Ñng khi chß╗ìn 1 tß╗çp duy nhß║Ñt.</p>
                </div>
                ...
                <div className="mt-4 border-t border-slate-800 pt-4">
                  ...
                  >
                    <span>{showAdvanced ? "Γû╝ T├╣y chß╗ìn n├óng cao" : "Γû╢ T├╣y chß╗ìn n├óng cao"}</span>
                  </button>

                  {showAdvanced && (
                    <div className="mt-3 space-y-4 animate-in fade-in slide-in-from-top-1 duration-200">
                      {/* Tß╗ÉI ╞»U ß║óNH TR╞»ß╗ÜC OCR */}
                      <div>
                        <label className="flex items-center space-x-2 text-slate-100 cursor-pointer select-none">
                          ...
                          />
                          <span className="text-xs font-bold uppercase tracking-wide">Tß╗æi ╞░u ß║únh tr╞░ß╗¢c OCR</span>
                        </label>
                        <p className="text-[10px] text-slate-400 mt-1 leading-relaxed">
                          Gi├║p OCR nhanh h╞ín vß╗¢i ß║únh chß╗Ñp dung l╞░ß╗úng lß╗¢n. Nß║┐u ß║únh bß╗ï mß╗¥ hoß║╖c kß║┐t quß║ú OCR k├⌐m, h├úy tß║»t t├╣y chß╗ìn n├áy.
                        </p>
                      </div>

                      {useImageOptimization && (
                        <div className="flex items-center space-x-2">
                          <label className="text-[10px] uppercase tracking-wide font-bold text-slate-300">Mß╗⌐c n├⌐n:</label>
                          <select
                          ...
                          >
                            <option value="balanced">C├ón bß║▒ng</option>
                      ...
                      <div className="flex items-start space-x-2.5 bg-slate-800 text-slate-300 p-3 rounded-lg border border-slate-700">
                        ...
                        <p className="text-[10px] leading-relaxed font-medium">
                          To├án bß╗Ö t├ái liß╗çu ─æ╞░ß╗úc xß╗¡ l├╜ theo kiß║┐n tr├║c Stateless. Hß╗ç thß╗æng kh├┤ng l╞░u hß╗ô s╞í sau khi kß║┐t th├║c OCR.
                        </p>
                      </div>
            ...
            <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm mt-4">
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between border-b border-slate-150 pb-3 mb-4">
                <h5 className="font-bold text-slate-850 text-sm sm:text-base flex flex-wrap items-center gap-2">
                  <span>Trang t├ái liß╗çu rß╗¥i rß║íc ─æ├ú ph├ón t├ích & tß╗▒ ─æß╗Öng n├⌐n ({totalPages} trang)</span>                  
                </h5>
                <button
                  type="button"
                  onClick={resetAll}
                  ...
                  title={isBatchProcessing ? "Kh├┤ng thß╗â x├│a khi ─æang OCR" : "L├ám lß║íi"}
                >
                  <Trash2 className="h-3.5 w-3.5" />
                  <span>X├│a tß║Ñt cß║ú</span>
                </button>
              </div>
              ...
              <div className="bg-slate-50 border border-slate-200 rounded-lg p-3.5 flex flex-wrap items-center justify-between gap-4 mb-5 text-sm font-semibold text-slate-700">
                <div className="flex flex-wrap items-center gap-4 sm:gap-6">
                  <div className="flex items-center">
                    <span className="text-slate-800">≡ƒôä {totalPages} trang</span>
                  </div>
                  <div className="h-4 w-px bg-slate-300 hidden sm:block" />
                  <div className="flex items-center">
                    <span className="text-slate-800">≡ƒÆ╛ {formatSize(totalOriginalBytes)}</span>
                  </div>
                  {useImageOptimization && hasOptimizedPages && (
                    <>
                      <div className="h-4 w-px bg-slate-300 hidden sm:block" />
                      <div className="flex items-center text-emerald-600">
                        <span>≡ƒôë Tiß║┐t kiß╗çm: {Math.max(0, 100 - Math.round((totalOptimizedBytes / totalOriginalBytes) * 100))}%</span>
                      </div>
                    </>
                  )}
                </div>
                {isBatchProcessing && (
                  <div className="flex items-center space-x-1.5 text-blue-600 font-bold bg-blue-50/50 px-2.5 py-1 rounded border border-blue-150 text-xs">
                    ...
                    <span>ΓÜí ╞»ß╗¢c t├¡nh OCR: ~{Math.ceil((totalPages - allPageItems.filter(p => p.state.status === 'success' || p.state.status === 'error').length) * 3)} gi├óy</span>
                  </div>
                )}
              </div>
                  ...
                  return (
                    ...
                    >
                      ...
                      <button
                        ...
                        title={isBatchProcessing ? "Kh├┤ng thß╗â x├│a khi ─æang OCR" : "X├│a khß╗Åi danh s├ích"}
                      >
                        <Trash2 className="h-3.5 w-3.5" />
                      </button>
                      ...
                      <div className="relative aspect-[3/4] bg-slate-100 flex items-center justify-center overflow-hidden border-b border-slate-150">
                        {pageImgUrl ? (
                          ...
                          <div className="flex flex-col items-center justify-center text-slate-400 space-y-1.5 p-4">
                            ...
                            <span className="text-[9px] text-center text-slate-500">─Éang chuß║⌐n bß╗ï trang...</span>
                          </div>
                        )}
                      </div>
                        ...
                        <div className="w-full mb-2">
                          ...
                          <div className="flex items-center justify-between mt-1.5">
                            ...
                            <span className={`text-[9px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-full ${
                              ...
                              {status === 'idle' && '≡ƒƒí ─ÉANG CHß╗£'}
                              {status === 'processing' && '≡ƒö╡ ─ÉANG OCR'}
                              {status === 'success' && '≡ƒƒó HO├ÇN TH├ÇNH'}
                              {status === 'error' && '≡ƒö┤ Lß╗ûI'}
                            </span>

                            {/* Action/Status Icon */}
                            <div className="flex items-center">
                              {status === 'success' && (
                                <span className="text-emerald-600 font-extrabold text-[11px]" title="Th├ánh c├┤ng">Γ£ô</span>
                              )}
                              {status === 'error' && (
                                <button
                                  type="button"
                                  onClick={() => setPageErrorDetail({ pageNum, error: error || "Lß╗ùi kh├┤ng x├íc ─æß╗ïnh" })}
                              ...
                              {status === 'idle' && (
                                <span className="h-1.5 w-1.5 bg-slate-350 rounded-full" title="Chß╗¥ tr├¡ch xuß║Ñt"></span>
                              )}
                            </div>
                          </div>
                        </div>
                          ...
                          {pageSizeInfo ? (
                            <>
                              ...
                              {pageSizeInfo.wasOptimized ? (
                                ...
                                <div className="flex justify-between">
                                  <span className="text-slate-400 font-sans">Sau n├⌐n:</span>
                                  <span className="font-semibold text-slate-500">Kh├┤ng n├⌐n</span>
                                </div>
                              )}
                            </>
                          ) : (
                            <div className="flex justify-between">
                              <span className="text-slate-400 font-sans">Gß╗æc:</span>
                              <span className="font-semibold text-slate-700">{slicedPage?.size || "ΓÇö"}</span>
                            </div>
                          )}
                        </div>
                      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="bg-white rounded-xl shadow-xl p-6 max-w-sm w-full animate-in fade-in duration-200">
            <div className="flex items-center space-x-2 text-red-600 mb-4">
              ...
              <h3 className="text-lg font-bold">Giß╗¢i hß║ín sß╗¡ dß╗Ñng</h3>
            </div>
            <div className="text-slate-600 text-sm mb-6 leading-relaxed space-y-3">
              {limitModal.type === "daily" ? (
                <>
                  {limitModal.maxAllowed <= 0 ? (
                    <p>Bß║ín ─æ├ú sß╗¡ dß╗Ñng hß║┐t 50 trang miß╗àn ph├¡ h├┤m nay.<br/><br/>Bß║ín c├│ thß╗â quay lß║íi v├áo ng├áy mai hoß║╖c n├óng cß║Ñp LexOCR PRO ─æß╗â tiß║┐p tß╗Ñc xß╗¡ l├╜ ngay.</p>
                  ) : (
                    <p>H├┤m nay bß║ín c├▓n <strong>{limitModal.maxAllowed}</strong> trang miß╗àn ph├¡.<br/><br/>H├úy giß║úm phß║ím vi xß╗¡ l├╜ xuß╗æng tß╗æi ─æa {limitModal.maxAllowed} trang hoß║╖c n├óng cß║Ñp LexOCR PRO.</p>
                  )}
                </>
              ) : (
                <p>
                  G├│i Free hß╗ù trß╗ú xß╗¡ l├╜ tß╗æi ─æa 20 trang trong mß╗ùi lß║ºn OCR.<br/><br/>
                  Bß║ín c├│ thß╗â chß╗ìn mß╗Öt phß║ím vi tß╗æi ─æa 20 trang, v├¡ dß╗Ñ 1ΓÇô20 hoß║╖c 21ΓÇô40, ─æß╗â tiß║┐p tß╗Ñc sß╗¡ dß╗Ñng miß╗àn ph├¡.<br/><br/>
                  N├óng cß║Ñp LexOCR PRO ─æß╗â xß╗¡ l├╜ to├án bß╗Ö t├ái liß╗çu trong mß╗Öt lß║ºn.
                </p>
              )}
            </div>
      ...
      {errorModalMsg && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="bg-white rounded-xl shadow-xl p-6 max-w-sm w-full animate-in fade-in duration-200">
            <div className="flex items-center space-x-2 text-red-600 mb-4">
              <AlertTriangle className="h-6 w-6" />
              <h3 className="text-lg font-bold">Th├┤ng b├ío</h3>
            </div>
            <p className="text-slate-600 text-sm mb-6 leading-relaxed">
              {errorModalMsg}
            </p>
            <div className="flex flex-col space-y-2">
              {typeof errorModalMsg === "string" && errorModalMsg.includes("hß║ín mß╗⌐c miß╗àn ph├¡") && (
                <button 
                  onClick={() => {
          ...
          <div className="bg-white rounded-xl shadow-xl p-6 max-w-lg w-full animate-in zoom-in-95 duration-200">
            <div className="flex items-center justify-between mb-4 border-b border-slate-100 pb-3">
              <div className="flex items-center space-x-2 text-rose-600">
                ...
                <h3 className="text-lg font-bold">Chi tiß║┐t lß╗ùi - Trang {pageErrorDetail.pageNum}</h3>
              </div>
              <button 
            ...
            <div className="bg-slate-50 p-3 rounded-lg border border-slate-200">
              <p className="text-xs text-slate-600 leading-relaxed">
                <strong>Gß╗úi ├╜ khß║»c phß╗Ñc:</strong> ─Éß╗â chß║íy lß║íi trang n├áy, bß║ín c├│ thß╗â thiß║┐t lß║¡p <strong>Tß╗½ trang: {pageErrorDetail.pageNum}</strong> v├á <strong>─Éß║┐n trang: {pageErrorDetail.pageNum}</strong> ß╗ƒ bß║úng <em>Phß║ím vi tr├¡ch xuß║Ñt (Page Range)</em> b├¬n phß║úi v├á nhß║Ñn Bß║»t ─æß║ºu lß║íi.
              </p>
            </div>
