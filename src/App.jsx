...
function App() {
  ...
  return (
    <div className="min-h-screen bg-[#fffcf7] text-[#0d0129] font-sans selection:bg-[#fae59b] selection:text-[#0d0129] flex flex-col scroll-smooth">
      <header className="sticky top-0 z-50 w-full bg-[#ffffff]/90 backdrop-blur-md border-b border-[#0d0129] transition-all select-none">
        <div className="max-w-[1280px] mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          <div className="flex items-center gap-2 cursor-pointer" onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })}>
            <div className="bg-[#fae59b] text-[#0d0129] p-2 rounded-[4px] border border-[#0d0129] font-black">
              ☕
            </div>
            <span className="font-black text-base text-[#0d0129] tracking-tight flex items-center gap-1.5">
              Gemini OCR <span className="bg-[#fae59b] text-[#0d0129] border border-[#0d0129] px-2 py-0.5 rounded-none text-[10px] font-black tracking-wider">1-LINE</span>
            </span>
          </div>

          <nav className="hidden md:flex items-center gap-6 text-xs font-bold text-[#0d0129]/80">
            <button onClick={() => scrollToSection('cong-cu')} className="hover:text-[#0d0129] hover:underline transition-colors cursor-pointer">Sử dụng</button>
            <button onClick={() => scrollToSection('so-sanh')} className="hover:text-[#0d0129] hover:underline transition-colors cursor-pointer">So sánh đối ứng</button>
            <button onClick={() => scrollToSection('faq')} className="hover:text-[#0d0129] hover:underline transition-colors cursor-pointer">Hỏi đáp & Bảo mật</button>
          </nav>

          <div>
            <button 
              onClick={() => scrollToSection('cong-cu')}
              className="px-5 py-2.5 text-xs font-bold bg-[#0d0129] hover:bg-[#19615c] text-white rounded-[4px] border border-[#0d0129] transition-all flex items-center gap-1.5 cursor-pointer shadow-sm"
            >
              Trải nghiệm ngay
              <ArrowRight size={14} />
            </button>
      ...
      <main className="max-w-[1280px] w-full mx-auto px-4 sm:px-6 lg:px-8 py-8 flex-grow flex flex-col gap-12">
        <section id="cong-cu" className="flex flex-col gap-6 animate-fade-in">
          <div className="text-center py-4 max-w-3xl mx-auto select-none">
            <div className="inline-flex items-center gap-1.5 px-3.5 py-1 bg-[#ffffff] border border-[#0d0129] rounded-none text-[11px] font-bold text-[#0d0129] mb-4 shadow-[1px_1px_3px_0px_rgba(0,0,0,1)]">
              <Sparkles size={12} className="text-[#19615c] fill-[#19615c] animate-pulse" />
              Ứng dụng Client-Side Bảo mật tuyệt đối - Không lưu trữ dữ liệu
            </div>
            <h1 className="text-2xl sm:text-3xl font-black text-[#0d0129] tracking-tight leading-tight mb-3">
          ...
          {files.length === 0 ? (
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 min-h-[500px]">
              <div className="lg:col-span-5 flex flex-col gap-6">
                ...
                <div className="flex flex-col gap-4 bg-[#ffffff] p-4 rounded-none shadow-[1px_1px_3px_0px_rgba(0,0,0,1)] border border-[#0d0129] flex-1 max-h-[550px] overflow-hidden">
                  <QueueList 
                    files={files} 
                  ...
                  <div className="pt-4 mt-auto border-t border-[#0d0129]/30 shrink-0">
                    <button
                      onClick={startOCR}
                      disabled={isProcessing || files.some(f => f.status === 'splitting') || !files.some(f => !f.isParentPdf && (f.status === 'waiting' || f.status === 'error'))}
                      className="w-full py-3 px-5 bg-[#19615c] hover:bg-opacity-95 text-white font-bold rounded-[4px] border border-[#0d0129] transition-all shadow-[1px_1px_3px_0px_rgba(0,0,0,1)] disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 cursor-pointer text-sm"
                    >
        ...
        <section id="so-sanh" className="flex flex-col gap-6 py-6">
          <div className="text-center max-w-3xl mx-auto select-none">
            <span className="inline-flex items-center gap-1.5 px-3 py-1 bg-[#fae59b] border border-[#0d0129] rounded-none text-[10px] font-black text-[#0d0129] uppercase tracking-wider mb-3">
              So Sánh Đối Ứng
            </span>
            <h2 className="text-2xl font-black text-[#0d0129] tracking-tight mb-2">
          ...
          <div className="bg-[#ffffff] rounded-none shadow-[1px_1px_3px_0px_rgba(0,0,0,1)] border border-[#0d0129] overflow-hidden">
            {/* Header Row */}
            <div className="grid grid-cols-1 md:grid-cols-2 bg-[#ffffff] border-b border-[#0d0129] select-none">
              <div className="p-5 flex items-center gap-3 md:border-r border-[#0d0129]">
                <span className="text-lg text-[#717171]">❌</span>
                <h3 className="text-sm font-extrabold text-[#0d0129]">Khó khăn & Nỗi đau của bạn</h3>
              </div>
              <div className="p-5 flex items-center gap-3">
                <span className="text-lg text-[#19615c]">✨</span>
                <h3 className="text-sm font-extrabold text-[#0d0129]">Giải pháp vượt trội từ App</h3>
              </div>
            </div>

            {/* Rows */}
            {/* Row 1 */}
            <div className="grid grid-cols-1 md:grid-cols-2 border-b border-[#0d0129]/30 hover:bg-[#fffcf7] transition-colors duration-200">
              <div className="p-6 md:border-r border-[#0d0129]/30 flex gap-3.5 items-start">
                <span className="text-base shrink-0 mt-0.5 select-none">❌</span>
                <p className="text-[#0d0129] text-xs font-semibold leading-relaxed">
              ...
              <div className="p-6 flex gap-3.5 items-start">
                <span className="bg-[#fae59b] border border-[#0d0129] p-1 rounded-none text-xs shrink-0 select-none flex items-center justify-center">💡</span>
                <p className="text-[#0d0129] text-xs font-bold leading-relaxed">
            ...
            {/* Row 6 */}
            <div className="grid grid-cols-1 md:grid-cols-2 hover:bg-[#fffcf7] transition-colors duration-200">
              <div className="p-6 md:border-r border-[#0d0129]/30 flex gap-3.5 items-start">
                <span className="text-base shrink-0 mt-0.5 select-none">❌</span>
                <p className="text-[#0d0129] text-xs font-semibold leading-relaxed">
        ...
        <section id="faq" className="flex flex-col gap-6 py-6 max-w-3xl mx-auto w-full">
          <div className="text-center select-none">
            <div className="inline-flex items-center gap-1.5 px-3 py-1 bg-[#fae59b] border border-[#0d0129] rounded-none text-[10px] font-black text-[#0d0129] mb-3">
              <Shield size={12} />
          ...
          <div className="bg-[#ffffff] border border-[#0d0129] rounded-none shadow-[1px_1px_3px_0px_rgba(0,0,0,1)] divide-y divide-[#0d0129]/30">
              ...
              return (
                ...
                >
                  <button
                    ...
                    className="w-full px-5 py-4 flex items-center justify-between text-left font-extrabold text-xs text-[#0d0129] hover:bg-[#fffcf7] transition-colors focus:outline-none cursor-pointer"
                  >
                    <span className="pr-4 leading-relaxed">{item.q}</span>
                    <ChevronDown 
                      size={16} 
                      className={`text-[#717171] shrink-0 transition-transform duration-300 ${isOpen ? 'rotate-180 text-[#0d0129]' : ''}`} 
                    />
                  </button>
                  
                  {isOpen && (
                    <div className="px-5 pb-4 pt-3 text-[#717171] text-xs font-semibold leading-relaxed bg-[#fffcf7]/50 border-t border-[#0d0129]/30 animate-in fade-in slide-in-from-top-1 duration-250 font-medium">
                      {item.a}
                    </div>
                  )}
                </div>
      ...
      <footer className="bg-[#ffffff] text-[#717171] py-8 border-t border-[#0d0129] text-center text-[11px] font-bold mt-12 select-none">
        <div className="max-w-[1280px] mx-auto px-4">
          <p className="mb-1 text-[#0d0129]">Gemini OCR Tiếng Việt Hàng Loạt Ra 1 Dòng - Phiên bản Client-Side Bảo Mật</p>
