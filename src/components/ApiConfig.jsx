...
export default function ApiConfig({ onConfigChange }) {
  ...
  return (
    <div className="bg-slate-900 border border-slate-800 w-full rounded-2xl">
      <div className="px-4 py-3 flex flex-col lg:flex-row lg:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2 text-white select-none">
          <div className="p-2 bg-slate-950 text-slate-300 border border-slate-800 rounded-xl">
            <KeyRound size={18} />
          </div>
          <h1 className="font-bold text-sm tracking-tight text-white">Điền API Key của bạn</h1>
        </div>
        ...
        <div className="flex flex-1 flex-col sm:flex-row items-center gap-3 w-full lg:justify-end">
          ...
          <div className="flex w-full sm:w-auto gap-2">
            <div className="relative flex-1 sm:w-64 group">
              <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none text-slate-550 group-focus-within:text-emerald-400">
                <KeyRound size={14} />
              </div>
              <input
                type="password"
                ...
                className="w-full pl-9 pr-3 py-2 bg-slate-950 border border-slate-800 rounded-xl text-xs focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 transition-all font-medium text-slate-200 placeholder-slate-600"
              />
            </div>
            ...
            <button
              ...
              disabled={!apiKey}
              className="px-3.5 py-2 bg-emerald-600 hover:bg-emerald-500 text-white rounded-xl text-xs font-semibold flex items-center gap-1.5 transition-all shadow-sm active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed shrink-0 cursor-pointer shadow-emerald-950/20"
          ...
          <div className="flex w-full sm:w-auto gap-2">
            <div className="relative flex-1 sm:w-48 group">
              ...
              <select
                ...
                className="w-full pl-9 pr-8 py-2 bg-slate-950 border border-slate-800 rounded-xl text-xs focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 transition-all font-medium text-slate-300 appearance-none cursor-pointer"
              >
            ...
            <button
              ...
              className="px-3 py-2 bg-slate-800 text-slate-300 hover:bg-slate-700 hover:text-white transition-all border border-slate-700 rounded-xl flex items-center justify-center disabled:opacity-50 cursor-pointer"
          ...
          {model === 'Khác' && (
            <input
              ...
              className="w-full sm:w-48 px-3 py-2 bg-amber-950/25 border border-amber-900/60 rounded-xl text-xs focus:outline-none focus:ring-2 focus:ring-amber-500/20 focus:border-amber-500 transition-all font-medium text-amber-200 placeholder-amber-600"
            />
          )}
      ...
      {showToast && (
        <div className="fixed bottom-6 right-6 z-50 flex items-center gap-2 bg-slate-900 border border-emerald-500/30 text-emerald-400 px-4 py-3 rounded-xl shadow-xl shadow-slate-950/80 animate-in fade-in slide-in-from-bottom-4 duration-300">
