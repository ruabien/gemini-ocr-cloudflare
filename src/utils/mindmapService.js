...
export const generateCaseMindmap = async (ocrText, templateKey, apiKeysPool, modelName, diagramType = 'tong_the', diagramFormat = 'hình luồng') => {
  ...
  if (!templateKey || !diagramType || !diagramFormat) {
    const err = new Error("Cấu hình sơ đồ tư duy không hợp lệ (thiếu mẫu vụ án, loại sơ đồ, hoặc hình thức).");
...
Mẫu án nghiệp vụ chính: ${templateKey === 'dan_su' ? 'Dân sự' : templateKey === 'hanh_chinh' ? 'Hành chính' : templateKey === 'hon_nhan' ? 'Hôn nhân & Gia đình' : templateKey === 'thi_hanh_an' ? 'Thi hành án' : templateKey === 'tuy_chinh' ? 'Tùy chỉnh' : 'Hình sự (Quy chuẩn Hướng dẫn 10)'}.
  `;
